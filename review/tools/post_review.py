...
The review agent's output is a JSON document (see prompts/review.md). This script
...
def gh(repo: str, *args: str, input_file: Path | None = None) -> str:
    cmd = ["gh", "api", *args]
    try:
        result = subprocess.run(
            cmd,
            stdin=input_file.open() if input_file else None,
            capture_output=True,
            text=True,
        )
    finally:
...
def load_review(path: Path) -> dict:
    ...
    return {"summary": review["summary"], "verdict": verdict, "comments": comments}
...
def post_per_comment(repo: str, pr: int, review: dict, mark: bool) -> None:
    ...
    post_review(repo, pr, {"summary": review["summary"], "verdict": review["verdict"], "comments": []})
